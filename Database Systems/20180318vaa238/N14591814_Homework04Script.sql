...
FROM PERSON, MANAGER, ENGINEER
WHERE PERSON.IDNUMBER = ENGINEER.IDNUMBER --all engineers
AND PERSON.IDNUMBER = MANAGER.IDNUMBER	--all managers
ORDER BY Person_IDNumber ASC;
...
FROM PERSON, ENGINEER
WHERE PERSON.IDNUMBER = ENGINEER.IDNUMBER --all engineers
AND ENGINEER.MENTOR IS NOT NULL --engineers with mentors
ORDER BY Person_Name ASC;
...
MINUS
SELECT DISTINCT PERSON.NAME AS Person_Name
FROM PERSON, ENGINEER, PARTICIPATES
WHERE PERSON.IDNUMBER = ENGINEER.IDNUMBER --all engineers
AND ENGINEER.IDNUMBER = PARTICIPATES.ENGINEERID	--engineers who participate in at least 1 team
ORDER BY Person_Name ASC;
...
SELECT DISTINCT PERSON.NAME AS Person_Name, COUNT(PERSON.NAME) AS Team_Number 
FROM PERSON, ENGINEER, PARTICIPATES
WHERE PERSON.IDNUMBER = ENGINEER.IDNUMBER --all engineers
AND ENGINEER.IDNUMBER = PARTICIPATES.ENGINEERID	--all engineers who participate in at least 1 team
...
WHERE PERSON.IDNUMBER = MANAGER.IDNUMBER --all managers
AND MANAGER.DEGREE = '2' --managers with degree '2'
AND MANAGER.IDNUMBER = PROJECT.MANAGERID --managers projects
ORDER BY Person_Name ASC;
...
--Projects that dont have teams/wrong matches
CREATE TABLE TEMP0 AS
SELECT PROJECT.NAME, INCLUDES.TEAM
...
--Projects with their matched teams
...
--Formatted projects with number of teams
CREATE TABLE ANSWER06 AS
SELECT DISTINCT TEMP1.NAME AS Project_Name, COUNT(TEMP1.NAME) AS Team_Number
FROM TEMP1
GROUP BY TEMP1.NAME
...
--highest level of engineer who participates in a team
CREATE TABLE TEMP2 AS
SELECT PARTICIPATES.TEAM AS Team_Name, MAX(ENGINEER.LVL) AS Highest_Level
FROM PARTICIPATES, ENGINEER
WHERE PARTICIPATES.ENGINEERID = ENGINEER.IDNUMBER --engineers who participate in a team
GROUP BY PARTICIPATES.TEAM;

--all teams with engineers that dont participate in them or that arent the highest level
CREATE TABLE TEMP3 AS
SELECT TEAM.NAME, TEMP2.Highest_Level
FROM TEAM, TEMP2
MINUS
SELECT TEAM.NAME, TEMP2.Highest_Level
FROM TEAM, TEMP2
WHERE TEAM.NAME = TEMP2.Team_Name;

--all teams with their highest engineer level
CREATE TABLE ANSWER07 AS
SELECT DISTINCT TEMP2.Team_Name, TEMP2.Highest_Level
FROM TEMP2
MINUS
SELECT DISTINCT *
FROM TEMP3
...
spool N14591814_Homework04Spool.txt;
